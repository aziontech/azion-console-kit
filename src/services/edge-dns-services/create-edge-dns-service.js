...
export const createEdgeDNSService = async (payload) => {
  const adaptPayload = adapt(payload)

  let httpResponse = await AxiosHttpClientAdapter.request({
    url: `${makeEdgeDNSBaseUrl()}`,
    method: 'POST',
    body: adaptPayload
  })
...
const parseHttpResponse = (httpResponse) => {
  switch (httpResponse.statusCode) {
    case 201:
      return {
        feedback: 'Your Edge DNS has been created',
        urlToEditView: `/edge-dns/edit/${httpResponse.body.results[0].id}`
      }
    case 400:
