...
export class CustomPageVersionService extends VersionServiceBase {
  ...
  async listConnectorDependenciesByVersion(customPageId, versionId) {
    const version = await this.loadVersion(customPageId, versionId)
    return extractCustomPageConnectorDependencies(version?.config?.pages ?? [])
  }
}
