...
export class LegacyInvoicesAdapter {
  ...
  static transformInvoiceToRow(invoice, statusMap = DEFAULT_STATUS_MAP) {
    ...
    return {
      ...
      status: map[LegacyInvoicesAdapter.resolveStatusKey(invoice?.status)] || map.NotCharged,
  ...
  static transformInvoicesList(invoices, statusMap = DEFAULT_STATUS_MAP) {
    ...
    return invoices.map((invoice) =>
      LegacyInvoicesAdapter.transformInvoiceToRow(invoice, statusMap)
    )
  }
}
