...
export const createFunctionInstanceService = async (payload) => {
  const { id } = payload
  let httpResponse = await AxiosHttpClientAdapter.request({
    url: `${makeEdgeApplicationBaseUrl()}/${id}/functions_instances`,
    method: 'POST',
    body: adapt(payload)
  })
...
const parseHttpResponse = (httpResponse, edgeApplicationId) => {
  switch (httpResponse.statusCode) {
    case 201:
      return {
        feedback: 'Your Function instance has been created',
        originKey: httpResponse.body.results,
        urlToEditView: `/edge-applications/edit/${edgeApplicationId}/functions_instance/edit/${httpResponse.body.results.origin_key}`
      }
    case 400:
