 ...
 * `draft` is the natural editing state; `cancelled` and `failed` are
 * recoverable terminal states where the user resumes work on the same draft.
 */
export const isEditable = (state) => ['draft', 'cancelled', 'failed'].includes(state)
 ...
 * - `cancelled` and `failed` inherit the same set as `draft` because they are
...
export const STATE_ACTIONS = {
  ...
  cancelled: ['SAVE', 'SAVE_AND_BUILD', 'NEW_DRAFT_FROM', 'DELETE'],
  failed: ['SAVE', 'SAVE_AND_BUILD', 'NEW_DRAFT_FROM', 'DELETE']
}
