...
watchEffect(async () => {
  ...
  if (user.value.client_id) {
    const { isDeveloperSupportPlan } = await ContractServices.loadContractServicePlan({
...
<template>
  <div class="flex flex-col h-[calc(100vh-3.5rem)]">
    <AzionAIChatBlock :user="user" :suggestionsOptions="suggestionsOptions">
      <template #chatControls="{ clearChat }">
        <div class="flex gap-3">
          <PrimeButton icon="pi pi-eraser" outlined class="surface-border h-8 w-8" aria-label="New chat"
            v-tooltip.bottom="'New chat'" @click="clearChat" />
          <PrimeButton icon="pi pi-external-link" outlined class="surface-border h-8 w-8"
            aria-label="Open a chat in new tab" v-tooltip.bottom="'Open a chat in new tab'" @click="openChatInNewTab" />
          <PrimeButton icon="pi pi-times" outlined class="surface-border h-8 w-8" aria-label="Close"
            v-tooltip.bottom="'Close'" @click="closeSidebar" />
        </div>
      </template>
