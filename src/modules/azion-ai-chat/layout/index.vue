  ...
  <Toolbar class="border-noround border-x-none w-full pl-6 pr-8 py-3 z-10 border-top-none">
    <template #start>
      <h3 class="text-color text-lg font-medium flex gap-3">
        Azion Copilot
        <PrimeTag v-tooltip.bottom="tooltipMessage" value="Preview" />
      </h3>
    </template>
    <template #end>
      <slot name="chatControls" :clearChat="clearChat" />
    </template>
  </Toolbar>
  <div class="flex flex-auto flex-col overflow-x-hidden p-6 pt-3 custom-scroll"
    :class="{ 'justify-center gap-6': !getStartConversation }" ref="chatContainer">
    <div class="self-stretch py-3 flex-col justify-start items-center gap-10 flex" v-if="!getStartConversation"
