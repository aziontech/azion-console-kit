<script setup>
      ...
      if (result.statusCode === 200) {
        ...
        if (database.status === 'creating') {
          toast.add({
            ...
            detail: 'This database is still being created. You will be redirected to monitor the process.',
            life: 4000
          })
  ...
  const loadTableDefinition = async (tableName) => {
    ...
    try {
      ...
      if (result.statusCode === 200 && result.body.results?.length > 0 && result.body.results[0].rows?.length > 0) {
        selectedTableDefinition.value = result.body.results[0].rows[0][0] || 'Table definition not found'
      } else {
  ...
  const deleteSelectedRows = () => {
    ...
    dialog.open(DeleteDialog, {
      data: {
        ...
        entityDeleteMessage: `The selected ${selectedCount} row${selectedCount > 1 ? 's' : ''} will be permanently deleted from the table. This action cannot be undone.`,
        onSuccess: async () => {
...
<template>
    ...
    <template #content>
      <div class="sql-interface h-full overflow-hidden">
        <div class="flex h-full gap-3 p-3">
          ...
          <div class="database-sidebar flex-shrink-0 bg-surface-0 border-round-lg border-1 surface-border" style="width: 320px;">
            <div class="h-full flex flex-column overflow-hidden">
              ...
              <div class="p-4 flex-grow-1 overflow-hidden flex flex-column">
                ...
                <div v-if="isLoadingTables" class="flex-grow-1 overflow-y-auto pr-1">
                  <div 
                    v-for="i in 3" 
                    :key="`skeleton-${i}`"
                  ...
                  >
                    <div class="flex items-center gap-3">
                      ...
                      <Skeleton class="h-4" style="width: 120px;" />
                    </div>
              ...
              <div class="flex-shrink-0 border-top-1 surface-border templates-section" :class="{ 'p-4': !isTemplatesCollapsed, 'px-4 py-3': isTemplatesCollapsed }">
                <div 
          ...
          <div class="flex-1 min-w-0">
            <div class="h-full flex flex-column gap-2 overflow-hidden">
              ...
              <div class="flex-shrink-0">
                <Accordion :activeIndex="isEditorCollapsed ? null : 0">
                  ...
                  <div class="flex justify-content-between align-items-center">
                    ...
                    <div v-if="queryResults.length > 0" class="flex align-items-center gap-3 text-sm text-color-secondary px-3 py-1 bg-surface-100 dark:bg-surface-700 border-round">
                      <!-- Tempo de execução real da API -->
                      <div v-if="getQueryStats(queryResults)?.duration" class="flex align-items-center gap-1">
                        <i class="pi pi-clock text-xs"></i>
                        <span>{{ getQueryStats(queryResults).duration }}ms</span>
                      </div>
                      <!-- Fallback para tempo calculado -->
                      <div v-else-if="executionTime > 0" class="flex align-items-center gap-1">
                        <i class="pi pi-clock text-xs"></i>
                        <span>{{ executionTime }}ms</span>
                      </div>
                      
                      <!-- Estatísticas de leitura e escrita -->
                      <div v-if="getQueryStats(queryResults)?.rowsRead !== undefined" class="flex align-items-center gap-1">
                        <i class="pi pi-eye text-xs"></i>
                        <span>{{ getQueryStats(queryResults).rowsRead }} read</span>
                      </div>
                      <div v-if="getQueryStats(queryResults)?.rowsWritten !== undefined" class="flex align-items-center gap-1">
              ...
              <!-- Results and History -->
              <div class="flex-grow-1 bg-surface-0 border-round-lg border-1 surface-border overflow-hidden">
                <TabView 
                ...
                >
                  <TabPanel>
                    ...
                    <div v-if="isExecutingQuery" class="results-container overflow-y-auto pr-2" :style="{ 'max-height': isEditorCollapsed ? 'calc(100vh - 300px)' : 'calc(100vh - 450px)' }">
                      <div class="mb-6">
                    ...
                    <div v-else-if="queryResults.length === 0" class="flex flex-col items-center justify-center h-full text-center p-8">
                    ...
                    <!-- Resultados -->
                    <div v-else class="results-container overflow-y-auto pr-2" :style="{ 'max-height': isEditorCollapsed ? 'calc(100vh - 300px)' : 'calc(100vh - 450px)' }">
                      <div v-for="(result, index) in queryResults" :key="index" class="mb-6">
                        <div class="flex justify-content-between align-items-center mb-3 p-3 bg-surface-50 dark:bg-surface-800 border-round-lg">
                          <h4 class="text-base font-semibold text-color flex items-center gap-2">
                            <i class="pi pi-list text-primary"></i>
                            {{ queryResults.length > 1 ? `Result ${index + 1}` : 'Query Results' }}
                            <Tag v-if="result.rows?.length" :value="`${result.rows.length} row${result.rows.length !== 1 ? 's' : ''}`" severity="success" class="text-xs" />
                          </h4>
                        ...
                        <div v-if="hasResultError(result)" class="text-center p-6 bg-surface-50 dark:bg-surface-800 border-round-lg border-1 surface-border">
                          ...
                          <p class="text-sm text-color-secondary font-mono bg-surface-100 dark:bg-surface-700 px-3 py-2 border-round inline-block">
  ...
  >
    <template #body>
      <div class="w-full flex flex-col gap-8 max-md:gap-6">
        ...
        <div v-if="isLoadingSchema" class="w-full">
          <!-- Header Section Skeleton -->
          <div class="flex flex-col gap-8 max-md:gap-6">
            <div class="max-w-screen-3xl mx-auto gap-4 w-full surface-section rounded-md border surface-border p-3 sm:p-8 flex-wrap min-w-[2rem]">
        ...
        <!-- Loaded Content -->
        <div v-else-if="selectedTableSchema" class="w-full">
          ...
          >
            <template #body>
              <div class="flex justify-content-between align-items-center w-full">
                <div class="flex align-items-center gap-2">
                  <Tag icon="pi pi-table" :value="`${selectedTableSchema.columns.length} columns`" severity="info" />
                </div>
                <Button
                  icon="pi pi-copy"
          ...
          <!-- Columns Section -->
          <InfoSection title="Columns" :loading="false">
            <template #body>
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full">
                <div
                ...
                >
                  <div class="flex flex-column gap-3">
                    ...
                    <div class="text-xs text-color-secondary">
                      <span class="font-mono bg-surface-100 dark:bg-surface-700 px-2 py-1 border-round">
                        {{ column[2] }}
                      </span>
                    </div>
  ...
  >
    <template #body>
      <div class="w-full flex flex-col gap-8 max-md:gap-6">
        <!-- Skeleton Loading State -->
        <div v-if="isLoadingDefinition" class="w-full">
          <!-- Header Section Skeleton -->
          <div class="flex flex-col gap-8 max-md:gap-6">
            <div class="max-w-screen-3xl mx-auto gap-4 w-full surface-section rounded-md border surface-border p-3 sm:p-8 flex-wrap min-w-[2rem]">
              <div class="flex items-center gap-3 mb-4">
        ...
        <!-- Loaded Content -->
        <div v-else-if="selectedTableDefinition" class="w-full">
          <!-- Header Section with Table Info -->
          <InfoSection 
          ...
          >
            <template #body>
              <div class="flex justify-content-between align-items-center w-full">
                <div class="flex align-items-center gap-2">
                  <Tag icon="pi pi-code" value="CREATE TABLE Statement" severity="info" />
                </div>
                <Button
                  icon="pi pi-copy"
          ...
          <InfoSection title="CREATE TABLE Statement" :loading="false">
            <template #body>
              <div v-if="selectedTableDefinition" class="sql-definition-display border-round-lg border-1 surface-border overflow-hidden w-full bg-surface-ground">
                <div class="sql-definition-header p-2 border-bottom-1 surface-border bg-surface-50 dark:bg-surface-800 flex justify-content-between align-items-center">
                ...
                <div class="sql-monaco-container border-round-lg overflow-hidden" style="height: 350px;">
                  <vue-monaco-editor
              ...
              <div v-else class="flex items-center justify-center h-24 text-color-secondary">
        ...
        <div v-else class="flex flex-col items-center justify-center p-8 text-center">
          ...
          <p class="text-color-secondary">Select a table and click "View Definition" from the menu to view its CREATE TABLE statement</p>
        </div>
      </div>
    </template>
  </InfoDrawerBlock>
  ...
  <Menu
    ...
    :pt="{
      menuitem: ({ context }) => ({
        'data-testid': `table-menu-item-${context.item?.label?.toLowerCase().replace(/\s+/g, '-')}`
      })
    }"
  />
  </div>
