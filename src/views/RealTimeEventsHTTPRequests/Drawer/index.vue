...
<template>
  <InfoDrawerBlock v-model:visible="showDrawer" title="More Details">
    <template #body>
      <div class="w-full flex flex-col gap-8">
        <InfoSection :title="details.host" :date="details.ts" :tags="hostTag">
          <template #body>
        ...
        <InfoSection title="Request and Response Data" :tags="serverProtocolTag">
          <template #body>
            <div class="grid grid-cols-3 w-full ml-[1px] gap-4 lg:gap-8">
              <PrimeButton outlined type="button" label="Request Time" badge="ms" v-tooltip="details.requestTime" />
              <PrimeButton outlined type="button" label="Bytes Sent" badge="bytes" v-tooltip="details.bytesSent" />
              <PrimeButton outlined type="button" label="Request Length" badge="lines"
        ...
        <InfoSection title="Origin Data" :tags="upstreamTag">
          <template #body>
            <div class="flex flex-col sm:flex-row gap-8">
              <BigNumber label="Upstream Bytes Received" sufix="bytes">{{ details.upstreamBytesReceived }}</BigNumber>
              <BigNumber label="Upstream Response Time" sufix="ms">{{ details.upstreamResponseTime }}</BigNumber>
              <BigNumber label="Upstream Bytes Sent" sufix="bytes">{{ details.upstreamBytesSent }}</BigNumber>
            </div>

            <Divider />

            <div class="w-full flex sm:flex-row flex-col gap-3">
              <TextInfo label="Upstream Addr" class="w-full sm:w-5/12 flex-1">{{ details.upstreamAddr }}</TextInfo>
              <TextInfo label="Upstream Status" class="w-full sm:w-5/12 flex-1">{{ details.upstreamStatus }}</TextInfo>
            </div>
          </template>
        </InfoSection>
        ...
        <InfoSection title="Secure Data" :tags="secureTag">
          <template #body>
