...
// ── Mocks ──
vi.mock('@/components/base/advanced-filter-system-v2/filterFields/filterRow/component', () => ({
  OPERATOR_MAPPING: {
    Eq: { label: '=' },
    Ne: { label: '≠' },
    Like: { label: 'like' },
...
describe('Requirement 13.2: Filter add/exclude/remove with URL hash persistence', () => {
  ...
  beforeEach(() => {
    ...
    ctx = useFilterActions({
      filterData,
      filterFields: ref([makeFilterField('host'), makeFilterFieldWithRange('status', 'Status')]),
  ...
  it('handleAddFilter calls onError for unknown field', () => {
    ...
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ severity: 'warn' }))
  })
...
describe('Requirement 13.5: loadMore pagination maintains correct offset', () => {
  ...
  it('useFieldStats handles data reset (new query) correctly', async () => {
    ...
    const data = ref([makeRow(1, 'ts1', { host: 'a.com' }), makeRow(2, 'ts2', { host: 'b.com' })])
    const { fieldStats } = useFieldStats({
      data,
      availableFields: ref([
        { label: 'host', value: 'host' },
