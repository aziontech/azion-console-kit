  ...
  <ContentBlock fillHeight>
    ...
    <template #content>
      ...
      <!-- Tab contents (KeepAlive preserves per-tab state).
           `flex-1 min-h-0 flex flex-col` makes this region consume the bounded
           height propagated by ContentBlock's `fillHeight` chain, so the active
           TabPanelBlock fills it and the events table scrolls INTERNALLY (the
           page no longer grows with the row count). -->
      <div class="mt-3 flex flex-1 flex-col min-h-0">
        <!-- Bounded cache (fix L1): cap KeepAlive at MAX_TOTAL_TABS, the maximum
             number of simultaneously-open tabs (combinedOpenTabs, including the
             pinned Events tab). Open tabs are always most-recently-used, so none
             is ever evicted (zero refetch); only closed tabs are bounded-evicted,
             releasing each closed tab's full row buffer instead of leaking it. -->
        <KeepAlive :max="MAX_TOTAL_TABS">
          <TabPanelBlock
            v-if="!activeTabId || isEventsTabId(activeTabId)"
            :key="eventsPanelKey"
...
<script setup>
  ...
  import { resolveTabNeighbor } from '@/views/RealTimeEventsV2/composables/utils/resolveTabNeighbor'
  ...
  // ── Tab limit (single ceiling-aware source of truth across all tab kinds) ──
  // Declared first so both useSessionManager and useEventsTabs admit and
  // restore through the SAME ceiling (req 2.1, 2.5). The `openTabs` computed
  // reads the sibling refs lazily (only when evaluated), so the forward
  // reference below is safe and avoids the init-order trap — no mutable holder.
  // eslint-disable-next-line prefer-const
  let eventsTabs
  // eslint-disable-next-line prefer-const
  let openTabs
  const { MAX_TOTAL_TABS, canOpenNewTab, totalTabCount, capForRestore } = useTabLimit({
    openTabs: computed(() => [
      { id: null },
      ...eventsTabs.value,
  ...
  const sessionManager = useSessionManager({
    ...
    router,
    toast,
    // Single ceiling-aware admission + restore, shared with Events tabs.
    canOpenNewTab,
    capForRestore,
    // Dashboard restore reserves the pinned tab + any additional Events tabs.
    reservedTabCount: () => eventsTabs.value.length,
    fallbackCopyDialogRef,
    // Combined visual order so internal close paths (deleteSession) resolve the
    // correct neighbor across ALL tab kinds — same seam as user-driven close.
    // Lazily read: combinedOpenTabs is declared below but only invoked at
    // delete-time, so no init-order issue.
    combinedTabOrder: () => combinedOpenTabs.value
  })
  const {
    activeTabId,
    panels,
  ...
  const eventsTabsManager = useEventsTabs({
    toast,
    totalTabCount,
    // Single ceiling-aware admission + restore, shared with Dashboard tabs.
    canOpenNewTab,
    capForRestore,
    activeTabId
  })
  ...
  eventsTabs = eventsTabsManager.eventsTabs
  ...
  // ── Collapsed Events TabPanelBlock (pinned + additional tabs share one node) ──
  // Key must reproduce the exact KeepAlive keys ('tab-events' / `tab-${id}`) so
  // per-tab caching semantics are preserved. Bindings switch per state: the
  // pinned tab carries share-view seeds; additional tabs carry pending state.
  const eventsPanelKey = computed(() =>
    activeTabId.value ? `tab-${activeTabId.value}` : 'tab-events'
  )
  const eventsPanelBindings = computed(() =>
    activeTabId.value
      ? { tabId: activeTabId.value, pendingViewState: getPendingViewState(activeTabId.value) }
      : {
          tabId: null,
          initialFilterState: pendingShareViewState.value?.filters,
          initialPageSize: pendingShareViewState.value?.pageSize,
          initialSelectedFields: pendingShareViewState.value?.selectedFields
        }
  )
  ...
  const handleCloseTab = (tabId) => {
    if (isEventsTabId(tabId)) {
      ...
      // Resolve the neighbor from the COMBINED visual tab order (req 2.2 / C4)
      // so a closed Dashboard tab activates the tab visually adjacent to it —
      // which may be an additional Events tab that openTabs cannot "see".
      const nextActiveId = resolveTabNeighbor(combinedOpenTabs.value, tabId)
      closeTab(tabId, nextActiveId)
    }
  }
...
<style scoped>
  ...
  /* Same policy for EVERY button inside Events v2 (Fields toggle, session
     toolbar, filter bar, view toggles, detail sidebar, …): the @aziontech
     theme paints `.p-button:focus { box-shadow: $focusShadow }`, which also
     fires on mouse-click. Scope is this view only (`[data-v] .p-button`), so
     the rest of the app is untouched. Keyboard focus keeps the ring via
     `:focus-visible` — accessibility preserved. */
