...
export function useSessionManager({
  ...
  const restoreTabs = () => {
    try {
      ...
      // Ceiling-aware restore (req 2.1): Dashboard tabs restore under the SAME
      // unified ceiling as admission. Reserve the pinned Events tab plus any
      // additional Events tabs already present in the shared tab list. Both the
      // injected and the fallback paths slice against the single MAX_TOTAL_TABS
      // ceiling, so there is no divergence between counting systems.
      const reserved = 1 + reservedTabCountFn()
      const restoreCap =
        typeof capForRestore === 'function'
          ? capForRestore(reserved)
          : Math.max(0, MAX_TOTAL_TABS - reserved)
