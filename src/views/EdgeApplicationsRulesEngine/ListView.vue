...
<template>
  <DrawerRulesEngine ref="drawerRulesEngineRef" :isApplicationAcceleratorEnabled="isApplicationAcceleratorEnabled"
    :isDeliveryProtocolHttps="isDeliveryProtocolHttps" :isImageOptimizationEnabled="isImageOptimizationEnabled"
    :listEdgeApplicationFunctionsService="listEdgeApplicationFunctionsService" :listOriginsService="listOriginsService"
    :clipboardWrite="clipboardWrite" :isLoadBalancerEnabled="isLoadBalancerEnabled"
    :listCacheSettingsService="listCacheSettingsService" :edgeApplicationId="edgeApplicationId"
    :createRulesEngineService="createRulesEngineService" :editRulesEngineService="editRulesEngineService"
    :loadRulesEngineService="loadRulesEngineService" :documentationService="documentationService"
    :hideApplicationAcceleratorInDescription="hideApplicationAcceleratorInDescription"
    :isEdgeFunctionEnabled="isEdgeFunctionEnabled" @onSuccess="reloadList" data-testid="rules-engine-drawer" />
  <FetchListTableBlock :lazy="false" ref="listRulesEngineRef" :reorderableRows="true" :columns="getColumns"
    :editInDrawer="openEditRulesEngineDrawer" :listService="listRulesEngineWithDecorator" @on-load-data="handleLoadData"
    :pt="{
      thead: { class: !hasContentToList && 'hidden' }
    }" emptyListMessage="No rules found." @on-before-go-to-edit="handleTrackEditEvent" @onReorder="checkOrderRules"
    :isReorderAllEnabled="removeReorderForRequestPhaseFirstItem" data-testid="rules-engine-list" :actions="actions"
    isTabs :apiFields="RULES_ENGINE_API_FIELDS" :defaultOrderingFieldName="''" :rowsPerPageOptions="[2000]">
    <template #addButton="{ reload, data }">
      <div class="flex gap-4" data-testid="rules-engine-add-button">
        <SelectButton v-model="selectedPhase" @change="reloadList" :options="PHASE_OPTIONS" :unselectable="true"
          data-testid="rules-engine-select-phase" />
        <PrimeButton icon="pi pi-plus" label="Rule" @click="openCreateRulesEngineDrawerByPhase"
          data-testid="rules-engine-create-button" />
        <PrimeButton icon="pi pi-save" label="Save order" data-testid="rules-engine-save-order-button"
    ...
    <template #noRecordsFound>
      <EmptyResultsBlock v-if="!hasContentToList" :title="titleEmptyState" :description="descriptionEmptyState"
        :createButtonLabel="selectedPhase" :documentationService="documentationService" :inTabs="true" :noBorder="true"
        data-testid="rules-engine-empty-results">
        <template #default>
          <PrimeButton class="max-md:w-full w-fit" @click="openCreateRulesEngineDrawerByPhase" severity="secondary"
            icon="pi pi-plus" label="Rule" data-testid="rules-engine-empty-results-create-button" />
        </template>
      </EmptyResultsBlock>
    </template>
  </FetchListTableBlock>
