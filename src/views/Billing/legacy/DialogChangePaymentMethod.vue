...
<script setup>
  ...
  import {
    useLegacyWalletMutations,
    legacyWalletQueryKey
  } from '@/composables/billing-legacy/useLegacyWallet'
  import { useToast } from '@aziontech/webkit/use-toast'

  defineOptions({ name: 'legacy-dialog-change-payment-method' })
  ...
  const { createSetupIntent, setDefault: setDefaultPaymentMethod } = useLegacyWalletMutations()

  const awaitDefaultPaymentMethod = async (
    paymentMethodId,
    { maxAttempts = 6, delayMs = 500 } = {}
  ) => {
    const key = legacyWalletQueryKey()
