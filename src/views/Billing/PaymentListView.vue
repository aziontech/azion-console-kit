<template>
  ...
  >
    <ListTable
      ...
      defaultOrderingFieldName="isDefault"
...
<script setup>
  ...
  const listCreditCards = async () =>
    PaymentMethodsAdapter.toWalletRows(await paymentMethodsService.listPaymentMethods())
  ...
  const setPaymentAsDefault = async (payment) => {
    try {
      await paymentMethodsService.setDefaultPaymentMethod(payment.id)
  ...
  const actionsRow = ref([
    ...
    },
    {
      ...
      service: (id) => paymentMethodsService.deletePaymentMethod(id)
    }
  ])
