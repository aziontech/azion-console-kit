...
const arbTsRange = fc
  .integer({ min: 0, max: SIX_MONTHS_MS - 1 })
  .chain((offsetBegin) =>
    fc
      .integer({ min: 1, max: SIX_MONTHS_MS })
      .map((offsetEnd) => ({
  ...
  it('when chart path returns kpis: null and showSummary is true, fallback KPI is applied', async () => {
    await fc.assert(
      fc.asyncProperty(arbDataset, arbTsRange, async (dataset, tsRange) => {
        ...
        expect(instance.kpis.value.supportsStatusBreakdown).toBe(expectedKpi.supportsStatusBreakdown)
...
describe('useEventsData KPI fallback — abort and error-swallow paths', () => {
  ...
  it('token cancellation: stale KPI result is not applied when filterData changes mid-flight', async () => {
    ...
    loadSummaryKpis
      .mockResolvedValueOnce({ total: 1, clientErrors: 0, serverErrors: 0, avgRequestTime: null, supportsStatusBreakdown: false, supportsRequestTime: false })
