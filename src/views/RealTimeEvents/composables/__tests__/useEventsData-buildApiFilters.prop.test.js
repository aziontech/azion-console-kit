...
const arbNonInField = fc
  .tuple(arbValueField, fc.constantFrom(...NON_IN_OPERATORS), fc.oneof(arbAlpha, fc.integer({ min: 1, max: 999 }).map(String)))
...
describe('useEventsData.buildApiFilters preservation (Property 2)', () => {
  ...
  it('property: when fields collide on the same valueField+operator the last value wins (today\'s behavior)', () => {
    fc.assert(
      fc.property(arbValueField, fc.constantFrom(...NON_IN_OPERATORS), arbAlpha, arbAlpha, (vf, op, a, b) => {
        const fields = [
