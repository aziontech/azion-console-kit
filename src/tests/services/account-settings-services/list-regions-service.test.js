...
import { listRegionsService } from '@/services/account-settings-services'
...
const fixtures = {
  mockResponse: {
    data: {
      country: [
        {
          regionSet: [{ name: 'Region1', geonameId: '5678' }]
        }
      ]
    }
  },
  formattedResponse: [{ name: 'Region1', geonameId: '5678' }],
  mockError: [{ message: 'Error' }]
}

const makeSut = () => {
  const sut = listRegionsService
...
describe('AccountSettingsServices', () => {
  it('should call API with correct params', async () => {
    ...
    const mockPayload = {
      query: `query country { country(geoid: 1234) { regionSet { name, geonameId } } }`
    }
