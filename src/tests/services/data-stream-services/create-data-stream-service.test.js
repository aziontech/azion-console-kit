...
import { createDataStreamService } from '@/services/data-stream-services'
...
const fixtures = {
  ...
  dataStreamCustomTemplateMock: {
    ...
    dataSet: '{"session_id":"$session_id",  "event_id":"$event_id"}',
    dataSource: 'http',
    domains: [[], []],
    endpoint: 'qradar',
    QRadarUrl: 'https://qradar-trial-abcdef.qradar.ibmcloud.com:123456'
...
const makeSut = () => {
  const sut = createDataStreamService
...
describe('DataStreamServices', () => {
  ...
  it('should return a feedback message on successfully created', async () => {
    ...
    const { sut } = makeSut()
    const data = await sut(fixtures.dataStreamMock)

    expect(data.feedback).toBe('Your data stream has been created')
  })
  ...
  it('should return a feedback message on successfully created with domains', async () => {
    ...
    const data = await sut(dataStreamMockWithDomains)
  ...
  it.each([
    ...
    const data = await sut(dataStreamEndpointMock)
