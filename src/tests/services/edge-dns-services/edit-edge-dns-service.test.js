...
import { editEdgeDNSService } from '@/services/edge-dns-services'
...
describe('EdgeDnsServices', () => {
  ...
  it('should return a feedback message on successfully updated', async () => {
    ...
    expect(feedbackMessage).toBe('Edge DNS has been updated')
  })

  it('Should return an API validation on update an invalid Edge DNS', async () => {
    const apiErrorMock = 'API validation error, this eDNS is invalid'
    vi.spyOn(AxiosHttpClientAdapter, 'request').mockResolvedValueOnce({
      statusCode: 400,
