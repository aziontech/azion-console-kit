<template>
  <div class="w-full" ref="ignoreClickOutside">
    <div class="relative">
      ...
      <div class="flex flex-col mt-2 gap-1" v-if="handleErrorsQuery.length">
        <small
...
const executeQuery = () => {
  const filter = AzionQueryLanguage.parse(
    query.value,
    suggestionsData.value,
    domains.value
  );
...
const handleInitialQuery = () => {
  query.value = AzionQueryLanguage.handleInicialQuery(
