<template>
  <div
    ...
    class="p-inputtext font-normal text-sm leading-4 w-full h-auto whitespace-pre font-code min-h-9 max-h-24 min-w-0 overflow-x-hidden overflow-y-hidden empty:bef ore:content-[attr(placeholder)] empty:before:block empty:before:text-[color:var(--input-placeholder-text-color)] empty:before:whitespace-nowrap empty:before:overf low-hidden empty:before:text-ellipsis empty:before:pointer-events-none"
    @input="handleInput"
