  ...
  <div class="fixed bottom-6 left-6 z-50">
    ...
    <transition name="fade">
      ...
      >
        ...
        <div class="p-0 overflow-y-auto" style="max-height: 60vh;">
          <div 
            v-for="task in tasks" 
            :key="task.id" 
    ...
    >
      <div class="flex flex-column gap-4">
        ...
        <div class="flex flex-column gap-2">
          <label for="feedback" class="text-color font-medium">
...
const checkCurrentRoute = () => {
  try {
    ...
    tasks.value.forEach(task => {
      ...
      const triggerMethod = (typeof trigger === 'object' && trigger.method) 
        ? trigger.method.toUpperCase() 
        : 'GET';
      ...
      if (currentNormalized.endsWith(triggerNormalized) && 
          triggerMethod === currentMethod) {
...
window.fetch = async function(...args) {
  ...
  if (response.ok) {
    try {
      ...
      tasks.value.forEach(task => {
        if (task.trigger?.url && url.includes(task.trigger.url) && 
