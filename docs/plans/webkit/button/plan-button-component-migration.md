...
**Didn't:**
- SaveButton never use the disabled with loading prop at same time

**Consuming Files (43+ views):**

All files using ActionBarTemplate or ActionBarBlock need event name updates:
- Change `@onSubmit="handler"` to `@click="handler"`
- Change `@onCancel="handler"` to `@click="handler"`
...
<CreateFormBlock :schema="validationSchema" :createService="service">
  <template #action-bar="{ onSubmit, onCancel, loading }">
    <ButtonSave :loading="loading" @click="onSubmit" />
