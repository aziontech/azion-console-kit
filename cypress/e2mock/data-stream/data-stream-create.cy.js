...
describe('Data Stream spec', { tags: ['@dev4'] }, () => {
  ...
  it('should successfully create a data stream with domain filters', () => {
    ...
    cy.intercept('GET', '/v4/data_stream/streams?ordering=name&page=1&page_size=10&fields=id%2Cname%2Cdata_source%2Cactive%2Cdata_set_id%2Cendpoint', {
      fixture: '/data-streaming/list-all.json'
    }).as('dataStreamList')

    cy.intercept('GET', '/v4/workspace/workloads?page=1&page_size=100&fields=id%2C+name', {
    ...
    cy.get(selectors.dataStream.templateDropdown).click()
    cy.get(selectors.dataStream.templateOption(0)).click()
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(3000)
    ...
    cy.intercept('GET', '/v4/workspace/workloads?ordering=name&page=2&page_size=100&fields=id%2C+name', {
    ...
    cy.intercept('POST', 'v4/data_stream/streams', (req) => {
      expect(req.body).to.have.property('endpoint')
      expect(req.body).to.have.property('data_set_id')
      expect(req.body.filters.workloads).to.be.an('array')
      expect(req.body.filters.workloads).to.include.members([
        1742384127,
        1739880189,
        1729539972,
        1741002069,
