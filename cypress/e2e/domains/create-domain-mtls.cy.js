...
describe('Domains spec', { tags: ['@dev3', '@xfail'] }, () => {
  ...
  it('should create and delete a domain with a mTLS digital certificate', () => {
    ...
    cy.intercept('GET', '/api/v4/digital_certificates/certificates?ordering=name&page=1&page_size=100&fields=id%2Cname&search=').as('getTrustedCACertificate')
    cy.intercept('GET', `/api/v4/digital_certificates/certificates?ordering=name&page=1&page_size=100&fields=id%2Cname&search=${digitalCertificateName}`).as('getTrustedCACertificateByName')
